...
public class Client extends JApplet implements Runnable {
			...
			try
			{
				...
				while (!versionInfoRecieved)
				{
					...
					if (!serverVersion.equals("") &&
							(serverVersion.substring(0,OpCodes.OPCODELENGTH)).equals(OpCodes.SENDVERSIONNUMBER))
					{
						int versionLength = Integer.parseInt(
								serverVersion.substring(OpCodes.OPCODELENGTH,OpCodes.OPCODELENGTH+1));
						serverVersion = serverVersion.substring(
								OpCodes.OPCODELENGTH+1,OpCodes.OPCODELENGTH+1+versionLength);
	...
	/**
	 * Message is sent to client to ask them to upgrade.
